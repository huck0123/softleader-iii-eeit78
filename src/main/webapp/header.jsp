...
#collapse-login {
	...
	<div id="header-wrapper">
			...
			<div id="custom-search-input">
				<div class="input-group col-md-8"
					...
					<input type="text" class="form-control input-lg"
						placeholder="請輸入關鍵字" id="header-name-search"/> <span class="input-group-btn">
		...
		<div class="collapse" id="collapse-login"">
			...
			<div class="container">
				<h2 style="padding-left: 20px;margin-bottom: 20px; margin-top: 30px">登入您的帳號</h2>
				<div class="col-md-6 col-sm-6" style="padding-left: 20px; padding-right: 20px">
					<p>
	...
	$(document).ready(
			function() {
						...
						function() {
							if ($(window).scrollTop() > 99) {
								...
								$('#nav-header').stop().addClass(
										'navbar-header-custom-scroll');
								$('#brand').stop().addClass('brand-scroll')
							} else {
								$('#header-wrapper').stop().removeClass(
										'scroll-header');
								$('#to-top').stop().hide(500);
								$('#nav-header').stop().removeClass(
										'navbar-header-custom-scroll');
								$('#brand').stop().removeClass('brand-scroll')
							}
						});
				...
				$('#header-search-btn').on('click',function(){
					location.href = '/softleader-iii-eeit78/campaign/campaignShow?nameSearch='+$('#header-name-search').val()+'&page=0';
				});
			});
